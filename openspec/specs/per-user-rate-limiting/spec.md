# Spec: Per-User Rate Limiting

## Purpose

Defines how rate limiting is applied across the application, providing per-user throttle buckets for authenticated requests and IP-based fallback for unauthenticated requests. Centralises tier configuration in a shared constant and exposes an opt-in decorator API so controllers remain free of throttle implementation details.
